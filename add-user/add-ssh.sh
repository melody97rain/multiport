...
export creditt=$(cat /root/provided 2>/dev/null || echo "")
export box=$(cat /etc/box 2>/dev/null || echo "32")
export line=$(cat /etc/line 2>/dev/null || echo "32")
export back_text=$(cat /etc/back 2>/dev/null || echo "40")
...
# read ports & info from log-install.txt
...
ovpn3="$(grep -w "OHP OpenVPN" ~/log-install.txt 2>/dev/null | cut -d: -f2 | tr -d ' ')"
ovpn4="$(grep -w "OpenVPN SSL" ~/log-install.txt 2>/dev/null | cut -d: -f2 | tr -d ' ')"
ohpssh="$(grep -w "OHP SSH" ~/log-install.txt 2>/dev/null | cut -d: -f2 | tr -d ' ')"
ohpdrop="$(grep -w "OHP Dropbear" ~/log-install.txt 2>/dev/null | cut -d: -f2 | tr -d ' ')"
wsdropbear="$(grep -w "Websocket SSH(HTTP)" ~/log-install.txt 2>/dev/null | cut -d: -f2 | tr -d ' ')"
wsstunnel="$(grep -w "Websocket SSL(HTTPS)" ~/log-install.txt 2>/dev/null | cut -d: -f2 | tr -d ' ')"
wsovpn="$(grep -w "Websocket OpenVPN" ~/log-install.txt 2>/dev/null | cut -d: -f2 | tr -d ' ')"
nsdomain1=$(cat /root/nsdomain 2>/dev/null || echo "")
pubkey1=$(cat /etc/slowdns/server.pub 2>/dev/null || echo "")
...
# Use full path for useradd and chage 
if /usr/sbin/id $Login >/dev/null 2>&1; then
...
====================================================================
CONFIG SSH WS
SSH 22      : $(cat /usr/local/etc/xray/domain 2>/dev/null):22@$Login:$Pass
SSH 80      : $(cat /usr/local/etc/xray/domain 2>/dev/null):80@$Login:$Pass
SSH 443     : $(cat /usr/local/etc/xray/domain 2>/dev/null):443@$Login:$Pass
SSH 1-65535 : $(cat /usr/local/etc/xray/domain 2>/dev/null):1-65535@$Login:$Pass
====================================================================
CONFIG OPENVPN
...
echo -e "SSH 22      : $(cat /usr/local/etc/xray/domain 2>/dev/null):22@$Login:$Pass"
echo -e "SSH 80      : $(cat /usr/local/etc/xray/domain 2>/dev/null):80@$Login:$Pass"
echo -e "SSH 443     : $(cat /usr/local/etc/xray/domain 2>/dev/null):443@$Login:$Pass"
echo -e "SSH 1-65535 : $(cat /usr/local/etc/xray/domain 2>/dev/null):1-65535@$Login:$Pass"
echo -e "\e[$line═══════════════════════════════════════════════════════\e[m"
