...
# Fungsi sahkan tempoh sah skrip
VALIDITY () {
...
# Periksa kebenaran berdasarkan IP
IZIN=$(curl -s https://raw.githubusercontent.com/${GitUser}/allow/main/ipvps.conf | awk '{print $5}' | grep $MYIP)
if [[ "$MYIP" = "$IZIN" ]]; then
